...
while abs(error) > tol && ite <= 40
    ...
    lambda = 1;
    A = solve2norm2(Ja,-(Jb*B+rk),-DP,-(-DP*alphaCk+C + V1), lambda, tol,A);
    B = solve2norm2(Jb,-(Ja*A+rk),-DP,-(-DP*betaCk+D + V2), lambda, tol,B);
    [C,costC] = IRLS_TV(DP*alphaCk+DP*A - V1,speye(m*n),mu1/lambda,m,n,tol,[],mask);
    [D,costD] = IRLS_TV(DP*betaCk+DP*B - V2,speye(m*n),mu2/lambda,m,n,tol,[],mask);
    ...
    if mod(ite,10)==1
        estACtv = reshape(C/5^2/100*8.686,[m,n]);
