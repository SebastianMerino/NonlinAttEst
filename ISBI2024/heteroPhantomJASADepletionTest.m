...
resultsDir = 'Q:\smerino\Nonlinearity\resultsJASA\phantomDepletionTest';
[~,~,~] = mkdir(resultsDir);
...
freq = 6; % Frequency for depletion
...
blockParams.blockSize = [20 30]*wl;
blockParams.overlap = 0.8;
blockParams.zlim = [0.5; 6]/100;
blockParams.xlim = [-1.8; 2]/100;

% Attenuation estimation parameters
blockParamsAcs.zInf = 0.5;
blockParamsAcs.zSup = 6;
blockParamsAcs.xInf = -1.8;
blockParamsAcs.xSup = 2;
blockParamsAcs.blocksize = [20 30]*wl;
blockParamsAcs.overlap = 0.8;

if freq == 5
freqL = 4e6; freqH = 6e6;         % 5 MHz
elseif freq == 6
freqL = 4.5e6; freqH = 6.5e6;     % 6 MHz
else
freqL = 5e6; freqH = 7e6;         % 7 MHz
end
...
% plot
spectrumzf = db(squeeze(mean((Sp+Sd)/2,2)));
spectrumzf = spectrumzf - max(spectrumzf(:));
imagesc(f,z_ACS,spectrumzf,[-60 0])
xlim([0,10])
