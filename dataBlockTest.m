...
% a = (0.1:0.001:1)'/db(exp(1)); % Attenuation in Np/cm/MHz^gamma
bona = 6:0.05:11;
b = 1 + bona/2;
...
model = goldratio.*(1-exp(-2*a.*f.^gamma.*z))./(f.^gamma.*z);
l2norm = sum((model-data).^2,3)/2;
figure("Position",[100 100 300 250]),
imagesc(goldratio,a*db(exp(1)),db(l2norm))
ax = gca;
ax.YDir = "normal";
colorbar
xlabel('Goldberg ratio')
ylabel('AC [dB/cm/MHz^\gamma]')
title("L2 norm")
hold on
% goldberg = b0/a0;
% aRatio = b/goldberg;
% % plot(bona,aRatio*db(exp(1)), "w--", 'LineWidth',2)
plot(gold0,a0*db(exp(1)), "wx", 'LineWidth',2)
hold off
