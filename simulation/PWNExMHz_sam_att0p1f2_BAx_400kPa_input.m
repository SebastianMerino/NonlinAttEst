...
function PWNExMHz_sam_att0p1f2_BAx_400kPa_input(baSam,freqMHz,pressurekPa)
baSam = str2double(baSam);
attSam = 0.1;
...
BonA_map = single(baSam*ones(Nx,Ny,Nz));
att_map = single(attSam*ones(Nx,Ny,Nz));
...
file_out = ['PWNE',num2str(round(tone_burst_freq/1e6)), ...
    'MHz_sam_att0p',num2str(round(attSam*100),"%02d"),...
    'f',num2str(alpha_power*10),...
    '_BA',num2str(round(baSam)),'_nc', num2str(tone_burst_cycles),...
