...
for ii = 1:6 % number of sims
    ...
    for ss = 1:length(source_strength_vector)
        ...
        [~,~,~] = mkdir(fullfile(pwd,'temp','in'));
        [~,~,~] = mkdir(fullfile(pwd,'temp','out'));
        [~,~,~] = mkdir(fullfile(pwd,'temp','param'));

        file_out = ['PWNE_5MHz_homo_BA',num2str(ba),...
            '_att0p',num2str(att*10,'%.1i'),'f1p2_nc3p5_',...
            num2str(source_strength/1000),'kPa'];

        input_args = {...
            'PMLInside', false, 'PMLSize', [pml_x_size, pml_y_size, pml_z_size], ...
            'DataCast', DATA_CAST, 'DataRecast', true, 'PlotSim', false,...
            'SaveToDisk',fullfile(pwd,'temp','in',file_out)};
        ...
        % 
        fs = 1/kgrid.dt;
        save(fullfile(pwd,'temp','param',file_out),'fs','c0','param'); 
        clear rf; clear transducer
    end
end
