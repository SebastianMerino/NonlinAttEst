...
% [FX,FZ] = gradient(estAClm,dzP);
% estACinst = FZ.*zP+estAClm;
% [Xmesh,Zmesh] = meshgrid(xP,zP(1:end));
% inc = Xmesh.^2 + (Zmesh-centerDepth).^2 < (radiusDisk-3e-3).^2;
% back = Xmesh.^2 + (Zmesh-centerDepth).^2 > (radiusDisk+3e-3).^2;
...
figure; imagesc(xP*1e3,zP(2:end)*1e3,estACinst); colorbar;
clim(attRange);
title('\alpha_0 in \alpha(f) = \alpha_0 \times f^2 dB/100')
axis image
colormap turbo; colorbar;
...
%% ???
% izP = round(zP(2:end)./dzP);
% factorq = izP(1)./izP ;
% P = sparse(tril(ones(m-2)));
% P = P./izP(2:end);
% P = kron(speye(n),P);
% estACcum = estACinst - estACinst(1,:).*factorq; 
% estACcum = estACcum(2:end,:);
% estACinst = IRLS_TV(estACcum(:),P,muLocal,m-2,n,tol,[],ones((m-2)*n,1));
% estACinst = reshape(estACinst,m-2,n);

%% ADMM
% Hyperparameters BEST
tol = 1e-3;
muAlpha = 0.1; muBeta = 0.001;
rho = 0.1;
maxIte = 200;

% Hyperparameters TEST
% tol = 1e-3;
% muAlpha = 0.001; muBeta = 0.1;
% rho = 1;
% maxIte = 200;
...
%% Testing again with DIFF for attenuation
% Hyperparameters BEST
tol = 1e-3;
muAlpha = 1; muBeta = 0.01;
rho = 0.1;
maxIte = 200;
...
Fid = []; Reg = []; Dual = [];
Fid(1) = 1/2*norm( modelFreq(u,zP,freqVec) - bzf(:) )^2;
Reg(1) = muAlpha*TVcalc_isotropic(DP2*u(1:m*n),m,n,mask) + ...
    muBeta*TVcalc_isotropic(DP*u(m*n+1:end),m,n,mask);
Dual(1) = 0;
ite  = 0;
error = 1;
...
while abs(error) > tol && ite < maxIte
    ...
    % Loss
    Fid(ite+1) = 1/2*norm( modelFreq(u,zP,freqVec) - bzf(:) )^2;
    Reg(ite+1) = muAlpha*TVcalc_isotropic(DP2*v(1:m*n),m,n,mask) + ...
        muBeta*TVcalc_isotropic(DP*v(m*n+1:end),m,n,mask);
    Dual(ite+1) = norm(u-v);
    fprintf("Ite: %01i, Fid: %.3f, Reg: %.3f, Dual: %.3f\n",...
        ite,Fid(ite+1),Reg(ite+1),Dual(ite+1))
    ...
    if mod(ite,10) ==1 && ite<50
        alphaArr = reshape(DP2*u(1:m*n),[m,n]);
        betaArr = reshape(DP*u(1+m*n:end),[m,n]);
        estACtv = alphaArr*NptodB/100;
        estBAtv = 2*(betaArr-1);
