...
dataDir = 'C:\Users\sebas\Documents\Data\Nonlinearity\01-Oct-24\bf_wideBand';
% refDir = 'C:\Users\sebas\Documents\Data\Nonlinearity\adriana2';
refDir = dataDir;
resultsDir = fullfile(dataDir,'results_7MHz');
[~,~,~] = mkdir(resultsDir);
targetFiles = dir([dataDir,'\*7MHz_uniform*_1.mat']);
...
att_ref = 0.3504*f.^1.1483/NptodB; 
...
% For looping
refFiles = dir(fullfile(refDir,'RFfn2_L11-5v_8v40v_7MHz_ref*.mat'));
refFiles = refFiles(2:3);
...
%%
up = sam1(1:length(sam1)/2,:);
down = sam1(length(sam1)/2:end,:);
figure,
hold on
[pxx,fpxx] = pwelch(up-mean(up),nz,nz-wz,nz,fs);
meanSpectrum = db(mean(pxx,2));
maxUp = max(meanSpectrum);
meanSpectrum = meanSpectrum - maxUp;
plot(fpxx/1e6,meanSpectrum)

[pxx,fpxx] = pwelch(down-mean(down),nz,nz-wz,nz,fs);
meanSpectrum = db(mean(pxx,2));
meanSpectrum = meanSpectrum - maxUp;
plot(fpxx/1e6,meanSpectrum)
hold off
