...
% Referencia con f2, sample con f1p2, modelo con fgamma
setup;
...
muAlpha = 1; muBeta = 1;
beta0 = 1+baInit/2;
alpha0 = alphaInit*freq^gamma *100/NptodB; % Np/m
...
estAClm = reshape(alphaArr/freq^gamma /100*NptodB,[m,n]);
...
for iFreq = 1:length(freqVec)
    ...
    [bz,xP,zP] = getMeasurements(medium,filterParams,blockParams,gammaRef);
    bzf(:,:,iFreq) = bz;
end
...
% Objective functions
Fid = []; Reg = []; Dual = [];
Fid(1) = 1/2*norm( modelFreq(u,zP,freqVec,gamma) - bzf(:) )^2;
...
while abs(error) > tol && ite < maxIte
    ...
    % Fidelity step
    u = optimNonLinearGNFreq(zP,freqVec,bzf(:), speye(2*m*n),v-w, rho, tol,u,gamma);
    ...
    % Loss
    Fid(ite+1) = 1/2*norm( modelFreq(u,zP,freqVec,gamma) - bzf(:) )^2;
